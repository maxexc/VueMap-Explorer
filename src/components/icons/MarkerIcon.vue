<script setup>
const props = defineProps({
  isActive: {
    type: Boolean,
    default: false
  },
  isDefault: {
    type: Boolean,
    default: false
  }
})
</script>

<template>
  <div class="relative flex items-center justify-center">
    <div
      v-if="props.isActive"
      class="absolute w-10 h-10 rounded-full bg-orange-300 opacity-40 ripple"
    ></div>
    <svg
      class="marker-icon"
      :class="{
        'active-marker': props.isActive,
        'default-marker': props.isDefault && !props.isActive
      }"
      width="25"
      height="32"
      viewBox="0 0 20 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M10.0002 -5.11874e-05C8.72364 -0.0599241 7.44863 0.147406 6.25679 0.608668C5.06496 1.06993 3.98256 1.77496 3.0789 2.67863C2.17523 3.5823 1.4702 4.66469 1.00894 5.85652C0.547675 7.04836 0.340344 8.32337 0.400217 9.59995C0.400217 12.6239 2.80022 15.5999 4.00022 16.7999C5.20022 17.9999 10.0002 23.9999 10.0002 23.9999C10.0002 23.9999 14.8002 17.9999 16.0002 16.7999C17.2002 15.5999 19.6002 12.6239 19.6002 9.59995C19.6601 8.32337 19.4528 7.04836 18.9915 5.85652C18.5302 4.66469 17.8252 3.5823 16.9215 2.67863C16.0179 1.77496 14.9355 1.06993 13.7436 0.608668C12.5518 0.147406 11.2768 -0.0599241 10.0002 -5.11874e-05ZM10.0002 13.4999C9.22887 13.4999 8.47484 13.2712 7.83349 12.8427C7.19214 12.4141 6.69227 11.805 6.39709 11.0924C6.10191 10.3798 6.02467 9.59562 6.17515 8.8391C6.32564 8.08257 6.69708 7.38766 7.2425 6.84223C7.78793 6.29681 8.48284 5.92537 9.23936 5.77489C9.99589 5.6244 10.7801 5.70164 11.4927 5.99682C12.2053 6.292 12.8144 6.79187 13.2429 7.43322C13.6715 8.07458 13.9002 8.8286 13.9002 9.59995C13.9002 10.1121 13.7993 10.6192 13.6033 11.0924C13.4074 11.5656 13.1201 11.9955 12.7579 12.3577C12.3958 12.7198 11.9659 13.0071 11.4927 13.2031C11.0195 13.3991 10.5124 13.4999 10.0002 13.4999Z"
        :fill="props.isDefault ? '#1E90FF' : 'url(#paint0_linear_17_758)'"
        stroke="white"
      />
      <defs>
        <linearGradient
          id="paint0_linear_17_758"
          x1="0.390137"
          y1="10.3942"
          x2="19.6104"
          y2="10.4008"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#FFA279" />
          <stop offset="1" stop-color="#F3743D" />
        </linearGradient>
      </defs>
    </svg>
  </div>
</template>

<style scoped>
.ripple {
  margin-bottom: -25px;
  animation: rippleEffect 2s infinite;
  z-index: -1;
}

.active-marker {
  animation: bounce 2s infinite ease-in-out;
}

.marker-icon {
  filter: drop-shadow(0px 4px 2px rgba(0, 0, 0, 0.25));
}

@keyframes rippleEffect {
  0% {
    transform: scale(0.5);
    opacity: 0.7;
  }
  100% {
    transform: scale(2.5);
    opacity: 0;
  }
}

@keyframes bounce {
  0%,
  100% {
    transform: translateY(0);
  }
  50% {
    transform: translateY(-2px);
  }
}
</style>
